...
const file=process.argv[2]||path.join(__dirname,'../../perfunde28f.html');
...
['backlink','kicker','tabs','panel-caso','panel-instrumento','panel-lab','panel-aval','fea','fees','vea','vees','fSV','fEF','fCoupling','presets','fVerdict','quiz','pontes','disclaimer','honestidade'].forEach(id=>ok('#'+id,!!$(id)));
...
ok('4 presets de agente',doc.querySelectorAll('#presets .preset').length===4);

console.log('— ALÇA PV · a mecânica (window.F28 / FSTATE) —');
ok('Sunagawa: SV>0 com Ees/Ea válidos',window.F28.strokeVolume(150,1.2,3)>0);
// baixar a pós-carga (Ea) sobe o SV
$('fea').value='3'; $('fea').dispatchEvent(new window.Event('input',{bubbles:true})); const svHi=window.FSTATE.SV;
$('fea').value='1'; $('fea').dispatchEvent(new window.Event('input',{bubbles:true})); const svLo=window.FSTATE.SV;
ok('baixar Ea (3→1) sobe o SV',svLo>svHi,'Ea3 '+svHi.toFixed(1)+' → Ea1 '+svLo.toFixed(1));
// aplicar milrinona (inodilatador) sobe Ees e baixa Ea
$('fea').value='3'; $('fea').dispatchEvent(new window.Event('input',{bubbles:true}));
$('fees').value='1.2'; $('fees').dispatchEvent(new window.Event('input',{bubbles:true})); const svBase=window.FSTATE.SV;
doc.querySelector('#presets .preset[data-ag="milrinona"]').dispatchEvent(new window.Event('click',{bubbles:true}));
ok('milrinona (inodilatador) sobe o SV',window.FSTATE.SV>svBase,svBase.toFixed(1)+' → '+window.FSTATE.SV.toFixed(1));
ok('readouts SV/EF/acoplamento preenchidos',/\d/.test(txt('fSV'))&&/%/.test(txt('fEF'))&&/\d/.test(txt('fCoupling')));
ok('veredito fala de pós-carga (Ea) e SV',/Ea/.test(txt('fVerdict'))&&/SV/.test(txt('fVerdict')));
...
ok('kicker 28F',/28F/.test(txt('kicker')));
ok('pontes M7/M24/M28',['perfunde7','perfunde24','perfunde28'].every(h=>$('pontes').innerHTML.includes(h)));
