...
Há uma segunda colisão, em `shock.js`: `applyPressor` existe em `m21` (sobe a RVS) e em
`m23` (soma tônus exógeno) com efeitos diferentes. O núcleo a resolve por **namespace**
(`septic.applyPressor` vs `composite.applyPressor`), tornando o engano de chamada
