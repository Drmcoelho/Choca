...
~16 peças (Caderno 0 + 15). Tabela completa, engines e âncoras de validação em `CHOQUE.md`. Resumo:

`0` Matemática do transporte (Caderno) → `1` CaO₂ → `2` curva como entrega → `3` débito cardíaco → `4` pré-carga & responsividade a volume (**Guyton — o engine-jóia**) → `5` pós-carga & ventrículo (alça PV) → `6` DO₂/VO₂ & extração → `7` **PAM = DC × RVS (núcleo)** → `8` hipovolêmico → `9` cardiogênico → `10` obstrutivo → `11` distributivo/séptico → `12` **choque críptico** → `13` os 4 perfis (radar) → `14` vasopressores/inotrópicos (**mecanismo, não dose**) → `15` capstone + tutor gráfico dinâmico.
